...
import (
	...
	"github.com/dwirijal/dwizzySCRAPE/internal/kanzenin"
	"github.com/dwirijal/dwizzySCRAPE/internal/store"
)

type kanzeninFetcher interface {
...
func run(ctx context.Context) error {
	...
	timeout, err := parseTimeoutOverride("KANZENIN_HTTP_TIMEOUT", cfg.HTTPTimeout)
	if err != nil {
		return err
	}
	fetcher := kanzenin.NewService(
		kanzenin.NewClient(cfg.KanzeninUserAgent, cfg.KanzeninCookie, timeout),
		cfg.KanzeninBaseURL,
	)
	contentStore := store.NewContentStore(db)

	report, err := runFullBackfill(ctx, fetcher, contentStore, contentStore, backfillOptions{
		StartLetter:          parseStringEnv("KANZENIN_START_LETTER", "."),
		MaxLetters:           parseNonNegativeInt("KANZENIN_MAX_LETTERS", 0),
		MaxPagesPerLetter:    parsePositiveInt("KANZENIN_MAX_PAGES_PER_LETTER", 100),
		MaxChaptersPerSeries: parseNonNegativeInt("KANZENIN_MAX_CHAPTERS_PER_SERIES", 0),
		MaxSeries:            parseNonNegativeInt("KANZENIN_MAX_SERIES", 0),
	})
	if err != nil {
		return err
	}
	...
	log.Printf(
		"kanzenin full backfill done: start_letter=%s last_letter=%s letters=%d catalog_pages=%d discovered_series=%d succeeded_series=%d failed_series=%d attempted_chapters=%d succeeded_chapters=%d failed_chapters=%d",
		report.StartLetter,
		report.LastLetter,
	...
	if len(report.Failures) > 0 {
		log.Printf("kanzenin failures recorded: %d", len(report.Failures))
	}
	return nil
}
...
func runFullBackfill(
	ctx context.Context,
	fetcher kanzeninFetcher,
	seriesWriter seriesWriter,
	chapterWriter chapterWriter,
	options backfillOptions,
) (fullBackfillReport, error) {
	if fetcher == nil {
		return fullBackfillReport{}, fmt.Errorf("kanzenin fetcher is required")
	}
	if seriesWriter == nil {
		return fullBackfillReport{}, fmt.Errorf("kanzenin series writer is required")
	}
	if chapterWriter == nil {
		return fullBackfillReport{}, fmt.Errorf("kanzenin chapter writer is required")
	}
	...
	for _, letter := range letters {
		...
		for page := 1; page <= options.MaxPagesPerLetter; page++ {
			...
			report.CatalogPages++
			log.Printf("kanzenin catalog page discovered: letter=%s page=%d items=%d", letter, page, len(items))
			...
			for _, item := range items {
				...
				if err != nil {
					...
					log.Printf("kanzenin series fetch failed: letter=%s page=%d slug=%s error=%s", letter, page, slug, err)
					continue
				}
				if err := seriesWriter.UpsertManhwaSeries(ctx, series); err != nil {
					report.FailedSeries++
					report.Failures[slug] = err.Error()
					log.Printf("kanzenin series store failed: letter=%s page=%d slug=%s error=%s", letter, page, slug, err)
					continue
				}

				report.SucceededSeries++
				log.Printf("kanzenin series synced: letter=%s page=%d slug=%s chapters=%d", letter, page, series.Slug, len(series.Chapters))
				...
				for _, chapterRef := range chapters {
					...
					if err != nil {
						...
						log.Printf("kanzenin chapter fetch failed: series=%s chapter=%s error=%s", series.Slug, chapterSlug, err)
						continue
					}
					if err := chapterWriter.UpsertManhwaChapter(ctx, chapter); err != nil {
						report.FailedChapters++
						report.Failures[chapterSlug] = err.Error()
						log.Printf("kanzenin chapter store failed: series=%s chapter=%s error=%s", series.Slug, chapterSlug, err)
						continue
					}

					report.SucceededChapters++
					log.Printf("kanzenin chapter synced: series=%s chapter=%s pages=%d", series.Slug, chapter.Slug, len(chapter.Pages))
				}
			}
		}
	}
