CREATE OR REPLACE FUNCTION public.refresh_anime_v2()
RETURNS void
LANGUAGE plpgsql
AS $$
BEGIN
    PERFORM public.refresh_media_lookup_dims_v2();
    ...
    IF EXISTS (
        ...
        WHERE n.nspname = 'public'
          AND c.relname = 'anime_list_v2_view'
          AND c.relkind IN ('r', 'p')
    ) THEN
        INSERT INTO public.anime_list_v2_view (
        ...
        DELETE FROM public.anime_list_v2_view v
        WHERE NOT EXISTS (
    ...
    IF EXISTS (
        ...
        JOIN pg_namespace n ON n.oid = c.relnamespace
        WHERE n.nspname = 'public'
          AND c.relname = 'anime_detail_v2_view'
          AND c.relkind IN ('r', 'p')
    ) THEN
        INSERT INTO public.anime_detail_v2_view (
        ...
        DELETE FROM public.anime_detail_v2_view v
        WHERE NOT EXISTS (
            SELECT 1 FROM public.anime_list a WHERE a.slug = v.slug
        );
    END IF;
    ...
    IF EXISTS (
        ...
        JOIN pg_namespace n ON n.oid = c.relnamespace
        WHERE n.nspname = 'public'
          AND c.relname = 'anime_meta_v2_view'
          AND c.relkind IN ('r', 'p')
    ) THEN
        INSERT INTO public.anime_meta_v2_view (slug, trailer_youtube_id, cast_json)
        SELECT
            a.slug,
        ...
        DELETE FROM public.anime_meta_v2_view v
        WHERE NOT EXISTS (
            SELECT 1 FROM public.anime_list a WHERE a.slug = v.slug
        );
    END IF;
    ...
    IF EXISTS (
        ...
        JOIN pg_namespace n ON n.oid = c.relnamespace
        WHERE n.nspname = 'public'
          AND c.relname = 'anime_episode_v2_view'
          AND c.relkind IN ('r', 'p')
    ) THEN
        INSERT INTO public.anime_episode_v2_view (
        ...
        DELETE FROM public.anime_episode_v2_view v
        WHERE NOT EXISTS (
