...
SET
  release_day = COALESCE(i.release_day, c.release_day),
  release_window = COALESCE(NULLIF(i.release_window, ''), c.release_window),
  release_timezone = COALESCE(NULLIF(i.release_timezone, ''), 'Asia/Seoul'),
  cadence = CASE
    WHEN COALESCE(i.cadence, '') = '' AND c.release_day IS NOT NULL THEN 'weekly'
    ELSE i.cadence
  END,
  updated_at = now()
