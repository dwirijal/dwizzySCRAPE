CREATE OR REPLACE FUNCTION public.refresh_movie_v3()
RETURNS void
LANGUAGE plpgsql
AS $$
BEGIN
    IF EXISTS (
        ...
        WHERE n.nspname = 'public'
          AND c.relname = 'movie_route_card_v3_view'
          AND c.relkind IN ('r', 'p')
    ) THEN
        INSERT INTO public.movie_route_card_v3_view (
            provider_record_id,
            provider_code,
            provider_movie_slug,
            provider_title,
        ...
        DELETE FROM public.movie_route_card_v3_view v
        WHERE NOT EXISTS (
    ...
    IF EXISTS (
        ...
        JOIN pg_namespace n ON n.oid = c.relnamespace
        WHERE n.nspname = 'public'
          AND c.relname = 'movie_route_detail_v3_view'
          AND c.relkind IN ('r', 'p')
    ) THEN
        INSERT INTO public.movie_route_detail_v3_view (
            provider_record_id,
            provider_code,
            provider_movie_slug,
            provider_title,
        ...
        DELETE FROM public.movie_route_detail_v3_view v
        WHERE NOT EXISTS (
            SELECT 1 FROM public.movie_provider_records pr WHERE pr.id = v.provider_record_id
        );
    END IF;
