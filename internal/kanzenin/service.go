...
func normalizeCatalogURL(baseURL, letter string, page int) string {
	...
	if page <= 1 {
		if letter == "" {
			return baseURL + "/a-z-list/"
		}
		return fmt.Sprintf("%s/a-z-list/?show=%s", baseURL, url.QueryEscape(letter))
	}
	if letter == "" {
		return fmt.Sprintf("%s/a-z-list/page/%d/", baseURL, page)
	}
	return fmt.Sprintf("%s/a-z-list/page/%d/?show=%s", baseURL, page, url.QueryEscape(letter))
}
